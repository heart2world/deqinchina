...
<body>
	<div class="wrap js-check-wrap">
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="<?php echo U('Course/banner');?>">页面轮播图</a></li>
		</ul>
        <form class="well form-search" method="post" action="<?php echo U('Course/banner');?>">
             标题:
            <input type="text" name="name" style="width: 100px;" value="<?php echo I('request.name/s','');?>">
            <input type="submit" class="btn btn-primary" value="查询" />
			<a class="btn btn-info" href="<?php echo U('Course/banneradd');?>">新增轮播图</a>
        </form>
			...
			<tbody>
				...
				<?php if(is_array($banner)): foreach($banner as $key=>$vo): ?><tr>
					...
					<td>
						...
						<a class="label label-inverse" href='<?php echo U("Course/banneredit",array("id"=>$vo["id"]));?>'><?php echo L('EDIT');?></a>
        ...
        function delete_banner(id) {
            $.dialog({
                ...
                ok: function(){
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo U("Admin/Course/banner_delete");?>',
                        data: {id:id},
                        dataType: 'json',
        ...
        function shelf(id) {
            $.dialog({
                ...
                ok: function(){
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo U("Admin/Course/shelf");?>',
                        data: {id:id},
                        dataType: 'json',
                        success: function (res) {
                            if(res.rs === 0){
        ...
        function obtained(id) {
            $.dialog({
                ...
                ok: function(){
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo U("Admin/Course/obtained");?>',
                        data: {id:id},
                        dataType: 'json',
                        success: function (res) {
                            if(res.rs === 0){
